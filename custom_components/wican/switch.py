...
async def async_setup_entry(
    ...
    has_steering_heater = False
    matching = []

    if catalog:
        entries = catalog.get("entries", catalog) if isinstance(catalog, dict) else catalog
        if isinstance(entries, list):
            matching = [item for item in entries if isinstance(item, dict) and _is_steering_wheel_heater_action(item)]
    ...
    def handle_catalog_update(webhook_id, data):
        ...
        matching = [item for item in cat_entries if isinstance(item, dict) and _is_steering_wheel_heater_action(item)]
